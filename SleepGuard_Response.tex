...
{\large\noindent \textbf{Summary of Minor Revisions -- SLEEPGUARD: Capturing Rich Sleep Information using Smartwatch Sensing Data}
\\
}
...
We thank the reviewers for their constructive feedback. In the revised manuscript, we have made all the changes suggested by the Associate
Editor. Specifically, we have made the following changes to the revised paper:

\begin{itemize}

\item We address the concerns of Reviewer 1 in the following ways. In Section 5 of the revised manuscript, we acknowledge the limitations
    of our evaluation and analysis, and discuss our approach with respect to medical-grade solutions. In Section 4.2.3, we added new
    experiments by applying the event detection algorithms of ``Sleep Hunter" to our smartwatch data, showing that our algorithms deliver
    better performance. In Section 6.3, we provide a discussion with respect to ``Sleep As Android" and ``Sleep Hunter".

\item In Section 4.1.3, we report how often the hand was placed on the Abdomen, Chest, and Head during sleep in our dataset;

\item In Section 4.1.4, we give the number of micro body movements for each of our test participants. We also  provide new diagrams
    (Figures 21 and 22) to report the accuracy of micro body movement detection of our approach.

\item In Table 9, we give both the mean score and standard deviation per testing user. We also show how close the estimation given by our
    approach and Fitbit to the user self-rating score in the same table. In Section 4.2.4, we provide a statistical analysis for the user
    study responses when discussing Table 9.

\item We have made all the minor corrections.
