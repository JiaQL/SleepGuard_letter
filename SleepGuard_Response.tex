...
\item We have acknowledged the limitations of our evaluation and analysis according to the feedback given by Reviewer 1 in
    Section~\ref{}. We also provided a discussion with respect to ``Sleep As Android" and ``Sleep Hunter" in \FIXME{Section~\ref{}};

\item In xx, we provide the percentages of the three hand positions, Abdomen, Chest, Head, during sleep in our pilot dataset;

\item In xx, we give the number of micro body movements for each of our test participants;

\item We now list the mean and standard deviation measurements in Table 9;
